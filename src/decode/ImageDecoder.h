...
// 内部是解码器注册表:按 magic bytes(必要时辅以 nameHint 的后缀,如
// RAW 家族无统一 magic)探测选择专用解码器,不匹配或专用解码失败时
// 兜底回退 QImageReader(自动应用 EXIF 方向)。解码失败返回空 QImage。
// nameHint 传来源的 entryName 即可,留空则只按 magic bytes 探测。
QImage decodeImage(const QByteArray& bytes, const QString& nameHint = QString());
